...
public class StatsCaching {
    ...
    public static Map<String, Map<UUID,Integer>> positions = new HashMap<>();
    ...
    public static void addPostions(List<PlayerStatsHolder> statsHolders, StatComparator comp, String name) {
        Map<UUID, Integer> positionsMap = new HashMap<>();
        for(int i = 0; i < statsHolders.size(); i++) {
            positionsMap.put(statsHolders.get(i).getUuid(), comp.get(statsHolders.get(i)));
        }
